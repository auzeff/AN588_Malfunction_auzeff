...
z.prop.test <- function(p1, n1, p2 = NULL, n2 = NULL, p0, alternative = "two.sided", conf.level = 0.95) {
  ...
  # Warnings
  if (n1 * p1 <= 5) {
    warning(paste("n1 * p1 =", n1 * p1, "-- normal approximation may not be valid"))
  }
  if (n1 * (1 - p1) <= 5) {
    warning(paste("n1 * (1 - p1) =", n1 * (1 - p1), "-- normal approximation may not be valid"))
  }
  # These warnings should only apply if p2 and n2 are defined
  if (!is.null(p2) && !is.null(n2)) {
    if (n2 * p2 <= 5) {
    warning(paste("n2 * p2 =", n2 * p2, "-- normal approximation may not be valid"))
    }
    if (n2 * (1 - p2) <= 5) {
    warning(paste("n2 * (1 - p2) =", n2 * (1 - p2), "-- normal approximation may not be valid"))
    }
  }
...
p <- p + xlab("Mean brain size (g)") + ylab("Max longevity (months)")  # modifying the axis labels
...
# Adding labels
p <- p + labs(x = "Mean brain size (g)", y = "Max longevity (months)", color = "Legend")
p
```
...
p <- p + xlab("Log mean brain size (g)") + ylab("Log max longevity (months)")
p <- p + geom_point()
...
# Adding labels
p <- p + labs(x = "Log mean brain size (g)", y = "Log max longevity (months)", color = "Legend")
p
```
